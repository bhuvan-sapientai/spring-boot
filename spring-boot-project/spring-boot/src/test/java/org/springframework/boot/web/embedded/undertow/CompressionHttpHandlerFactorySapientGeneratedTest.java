...
class CompressionHttpHandlerFactorySapientGeneratedTest {
	...
	//Sapient generated method id: ${4d2dfd2c-fa93-3fb7-b24b-408fe7e662c0}
	@Test()
	...
	//Sapient generated method id: ${f2eddcf4-047f-3e4d-a740-fdab8d20e36e}
	@Disabled()
	@Test()
	void getHandlerWhenCompressionGetExcludedUserAgentsIsNotEmpty() {
		 ...
		 *  Following variables could not be isolated/mocked: io.undertow.server.handlers.encoding.EncodingHandler, repository
		 *  Suggestions:
		...
		try (MockedStatic<Predicates> predicates = mockStatic(Predicates.class)) {
			...
			predicates.when(() -> Predicates.regex((RequestHeaderAttribute) any(), eq("excludedUserAgentsItem1"))).thenReturn(predicateMock);
			...
			EncodingHandler encodingHandler2 = encodingHandler.setNext(httpHandlerMock);
			//Assert statement(s)
			//TODO: Please implement equals method in EncodingHandler for verification to succeed or you need to adjust respective assertion statements
			assertAll("result", () -> {
				assertThat(result, equalTo(encodingHandler2));
				verify(dataSizeMock).toBytes();
				predicates.verify(() -> Predicates.regex((RequestHeaderAttribute) any(), eq("excludedUserAgentsItem1")));
