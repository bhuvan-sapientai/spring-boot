...
class ConfigDataImporterSapientGeneratedTest {
	...
	private final ConfigDataLocationResolvers resolversMock = mock(ConfigDataLocationResolvers.class, "ConfigDataResource");

	private final ConfigDataLoaders loadersMock = mock(ConfigDataLoaders.class, "ConfigDataLocation");

	private final ConfigDataActivationContext activationContextMock = mock(ConfigDataActivationContext.class, "true");
	...
	void resolveAndLoadWhenLoadedIsNotNull() throws IOException {
		 ...
		 */
		//Arrange Statement(s)
		ConfigDataActivationContext activationContextMock = mock(ConfigDataActivationContext.class);
		...
		ConfigDataResolutionResult configDataResolutionResultMock = mock(ConfigDataResolutionResult.class);
	...
	void resolveAndLoadWhenResourceIsNullAndILessThan0() throws IOException {
		 ...
		 */
		//Arrange Statement(s)
		Profiles profilesMock = mock(Profiles.class, "profilesValue");
		doReturn(profilesMock).when(activationContextMock).getProfiles();
		DeferredLogFactory logFactoryMock = mock(DeferredLogFactory.class, "logFactoryValue");
		//TODO: Needs to return real value
		doReturn(null).when(logFactoryMock).getLog(ConfigDataImporter.class);
		ConfigDataImporter target = new ConfigDataImporter(logFactoryMock, ConfigDataNotFoundAction.FAIL, resolversMock, loadersMock);
		ConfigDataResolutionResult configDataResolutionResultMock = mock(ConfigDataResolutionResult.class, "resolveValue");
		List<ConfigDataResolutionResult> configDataResolutionResultList = new ArrayList<>();
		configDataResolutionResultList.add(configDataResolutionResultMock);
		ConfigDataLocationResolverContext configDataLocationResolverContextMock = mock(ConfigDataLocationResolverContext.class, "locationResolverContextValue");
		...
		ConfigDataLoaderContext configDataLoaderContextMock = mock(ConfigDataLoaderContext.class, "loaderContextValue");
	...
	@Disabled()
	@Test()
	void resolveAndLoadWhenLoadedIsNotNullAndCaughtIOExceptionThrowsIllegalStateException() {
		/* Branches:
		 * (activationContext != null) : true
		 * (for-each(locations)) : true  #  inside resolve method
		 * (i >= 0) : true  #  inside load method
		...
		//Arrange Statement(s)
		doReturn(profilesMock).when(activationContextMock).getProfiles();
		DeferredLogFactory logFactoryMock = mock(DeferredLogFactory.class, "LogMessage");
		//TODO: Needs to return real value
		doReturn(null).when(logFactoryMock).getLog(ConfigDataImporter.class);
		ConfigDataImporter target = new ConfigDataImporter(logFactoryMock, ConfigDataNotFoundAction.FAIL, resolversMock, loadersMock);
		List<ConfigDataResolutionResult> configDataResolutionResultList = new ArrayList<>();
		ConfigDataLocationResolverContext configDataLocationResolverContextMock = mock(ConfigDataLocationResolverContext.class, "Profiles");
		doReturn(configDataResolutionResultList).when(resolversMock).resolve(eq(configDataLocationResolverContextMock), (ConfigDataLocation) any(), eq(profilesMock));
		ConfigDataLoaderContext configDataLoaderContextMock = mock(ConfigDataLoaderContext.class, "List<ConfigDataResolutionResult>");
		ConfigDataLocation configDataLocation = ConfigDataLocation.of("location1");
		List<ConfigDataLocation> configDataLocationList = new ArrayList<>();
		configDataLocationList.add(configDataLocation);
		IOException iOException = new IOException();
