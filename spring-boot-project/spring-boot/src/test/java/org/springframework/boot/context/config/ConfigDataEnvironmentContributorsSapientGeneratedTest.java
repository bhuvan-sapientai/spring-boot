...
class ConfigDataEnvironmentContributorsSapientGeneratedTest {
	...
	//Sapient generated method id: ${75f58abd-955f-3a73-8bad-89c5b631cdff}, hash: BC6FD521EB630C131938296F4802F27C
	@Disabled()
	@Test()
	...
	//Sapient generated method id: ${0813946b-1918-3824-85f7-5653ed308975}, hash: D036E63414FD06B32FC5EEA319D5428B
	@Disabled()
	@Test()
	void withProcessedImportsWhenIGreaterThanOrEqualsTo0() {
		...
		ConfigDataActivationContext configDataActivationContextMock = mock(ConfigDataActivationContext.class, "ConfigDataActivationContext");
		...
		try (MockedStatic<ConfigDataEnvironmentContributor> configDataEnvironmentContributor = mockStatic(ConfigDataEnvironmentContributor.class);
			...
			configDataEnvironmentContributorImportPhase.when(() -> ConfigDataEnvironmentContributor.ImportPhase.get(configDataActivationContextMock)).thenReturn(ConfigDataEnvironmentContributor.ImportPhase.BEFORE_PROFILE_ACTIVATION);
			logMessage.when(() -> LogMessage.format("Processing imports for phase %s. %s", ConfigDataEnvironmentContributor.ImportPhase.BEFORE_PROFILE_ACTIVATION, configDataActivationContextMock)).thenReturn(logMessageMock);
			LogMessage logMessage10 = LogMessage.format("Processing imports %s", configDataLocationList);
			...
			logMessage.when(() -> LogMessage.format(eq("Processing imports %s"), anyList())).thenReturn(logMessage10).thenReturn(logMessageMock3).thenReturn(logMessageMock5).thenReturn(logMessageMock7);
			logMessage.when(() -> LogMessage.of((Supplier) any())).thenReturn(logMessageMock2).thenReturn(logMessageMock4).thenReturn(logMessageMock6).thenReturn(logMessageMock8);
