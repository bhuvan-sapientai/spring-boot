...
class RandomValuePropertySourceSapientGeneratedTest {
	...
	//Sapient generated method id: ${09c5b2c4-a3dd-37b7-a254-a6c85c44ac03}, hash: 9EE76D7A1772BB6FED670E4FA1587721
	@Disabled()
	@Test()
	void getPropertyWhenRangeIsNotNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class, CALLS_REAL_METHODS);
			...
			doReturn(intStream).when(randomMock).ints(1L, 1, 1);
			//Act Statement(s)
			...
			assertAll("result", () -> {
				...
				verify(target, atLeast(1)).getSource();
				verify(randomMock, atLeast(1)).ints(1L, 1, 1);
			});
		}
	}

	//Sapient generated method id: ${c3f8f3ae-6ab4-3435-b568-f8708d2d87c9}, hash: A12B9384281F17E8A57F2324EF66F5C4
	@Disabled()
	@Test()
	void getPropertyWhenRangeIsNullAndRangeIsNotNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class, CALLS_REAL_METHODS);
			...
			doReturn(longStream).when(randomMock).longs(1L, 0L, 0L);
			//Act Statement(s)
			...
			assertAll("result", () -> {
				...
				_assert.verify(() -> Assert.state(eq(false), (Supplier) any()), atLeast(1));
				verify(target, atLeast(1)).getSource();
				verify(randomMock, atLeast(1)).longs(1L, 0L, 0L);
			});
		}
	}
