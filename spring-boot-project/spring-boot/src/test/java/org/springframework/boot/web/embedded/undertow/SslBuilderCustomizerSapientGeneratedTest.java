...
class SslBuilderCustomizerSapientGeneratedTest {
	...
	//Sapient generated method id: ${f26132c4-ec90-302a-9165-06d92ddb4548}
	@Disabled()
	@Test()
	void customizeWhenOptionsGetCiphersIsNotNull() {
		...
		try (MockedStatic<Sequence> sequence = mockStatic(Sequence.class)) {
			doReturn(undertowBuilderMock).when(builderMock).addHttpsListener(0, "0.0.0.0", (SSLContext) null);
			...
			doReturn(null).when(sslBundleMock).createSslContext();
			//Act Statement(s)
			target.customize(builderMock);
			//Assert statement(s)
			assertAll("result", () -> {
				verify(builderMock).addHttpsListener(0, "0.0.0.0", (SSLContext) null);
	...
	//Sapient generated method id: ${803e875d-1e76-3a14-b9d3-12a32879f704}
	@Test()
	void customizeWhenThisAddressIsNotNullAndOptionsGetEnabledProtocolsIsNullAndOptionsGetCiphersIsNull() {
		 ...
		 */
		//Arrange Statement(s)
		doReturn(undertowBuilderMock).when(builderMock).addHttpsListener(0, "127.0.0.1", (SSLContext) null);
		Option option = Options.SSL_CLIENT_AUTH_MODE;
		doReturn(undertowBuilderMock2).when(builderMock).setSocketOption(option, SslClientAuthMode.REQUIRED);
		...
		//Assert statement(s)
		assertAll("result", () -> {
			verify(builderMock).addHttpsListener(0, "127.0.0.1", (SSLContext) null);
			verify(builderMock).setSocketOption(option, SslClientAuthMode.REQUIRED);
			verify(sslBundleMock).getOptions();
