...
class LoggingSystemPropertiesSapientGeneratedTest {
	...
	private final BiConsumer<String, String> setterMock = mock(BiConsumer.class, "mockSetter");
	...
	//Sapient generated method id: ${762de8fd-c6de-3880-b4c4-2107fc7888f3}, hash: D0351BBA60CAE1A58AE9D9A9D5DAC44C
	@Test()
	void setSystemProperty4WhenValueIsNotNull() {
		 ...
		 */
		//Arrange Statement(s)
		PropertyResolver resolverMock = mock(PropertyResolver.class, "mockResolver");
		doReturn("mockValue").when(resolverMock).getProperty("mockPropertyName");
		Environment environmentMock = mock(Environment.class, "mockEnvironment");
		Function<String, String> functionMock = mock(Function.class, "mockDefaultValueResolver");
		LoggingSystemProperties target = new LoggingSystemProperties(environmentMock, functionMock, setterMock);
		doNothing().when(setterMock).accept("mockSystemPropertyName", "mockValue");

		//Act Statement(s)
		target.setSystemProperty(resolverMock, "mockSystemPropertyName", "mockPropertyName", "mockDefaultValue");

		//Assert statement(s)
		assertAll("result", () -> {
			verify(resolverMock).getProperty("mockPropertyName");
			verify(setterMock).accept("mockSystemPropertyName", "mockValue");
		});
	}
	...
	//Sapient generated method id: ${5518f6a9-6700-3de3-bdd8-17ff5a09c416}, hash: 5BF0A53CF0FEC5703A2B719C0C2B5FDB
	@Test()
	void setSystemProperty5Test() {
		//Arrange Statement(s)
		Function<String, String> functionMock = mock(Function.class);
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			_assert.when(() -> Assert.notNull(environmentMock, "Environment must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			LoggingSystemProperties target = new LoggingSystemProperties(environmentMock, functionMock, setterMock);
