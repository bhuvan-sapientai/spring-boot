...
class StringToDurationConverterSapientGeneratedTest {
	...
	//Sapient generated method id: ${d5fe6cef-93d9-323d-84c3-b6bbfb0f2d3f}, hash: BC9E43C46DBE75240864E5185543722C
	@Test()
	void convertWhenAnnotationIsNullAndAnnotationIsNullAndStyleIsNull() {
		...
		try (MockedStatic<DurationStyle> durationStyle = mockStatic(DurationStyle.class, CALLS_REAL_METHODS);
			...
			durationStyle.when(() -> DurationStyle.detect("convert_object1")).thenReturn(DurationStyle.SIMPLE);
			StringToDurationConverter target = new StringToDurationConverter();
			//Act Statement(s)
			...
			assertAll("result", () -> {
				...
				durationStyle.verify(() -> DurationStyle.detect("convert_object1"), atLeast(1));
			});
		}
	}
}
