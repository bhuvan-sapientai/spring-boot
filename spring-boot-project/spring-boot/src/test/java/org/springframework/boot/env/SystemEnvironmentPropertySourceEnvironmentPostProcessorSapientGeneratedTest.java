...
class SystemEnvironmentPropertySourceEnvironmentPostProcessorSapientGeneratedTest {
	...
	//Sapient generated method id: ${c38a95fc-a6cb-397a-a3de-fea67cb0dbba}
	@Test()
	void postProcessEnvironmentWhenPropertySourceIsNotNull() {
		...
		doNothing().when(mutablePropertySourcesMock2).replace(eq("systemEnvironment"), (SystemEnvironmentPropertySourceEnvironmentPostProcessor.OriginAwareSystemEnvironmentPropertySource) any());
		...
		assertAll("result", () -> {
			...
			verify(mutablePropertySourcesMock2).replace(eq("systemEnvironment"), (SystemEnvironmentPropertySourceEnvironmentPostProcessor.OriginAwareSystemEnvironmentPropertySource) any());
		});
	}
}
