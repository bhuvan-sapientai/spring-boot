...
class JasperInitializerSapientGeneratedTest {
	...
	//Sapient generated method id: ${900a33c3-798a-3566-95c7-ae71a17e071c}, hash: 9E703712B13F4BFA9432795DC401C77F
	@Disabled()
	@Test()
	void doStartWhenClassUtilsIsPresentOrg_apache_catalina_webresources_TomcatURLStreamHandlerFactoryGetClassGetClassLoader() throws Exception, ServletException, ClassNotFoundException, Error {
		...
		try (MockedStatic<TomcatURLStreamHandlerFactory> tomcatURLStreamHandlerFactory = mockStatic(TomcatURLStreamHandlerFactory.class);
			...
			ClassLoader classLoader = contextMock.getClassLoader();
			doReturn(classLoader).when(contextMock).getClassLoader();
			ServletContextHandler.ServletScopedContext servletScopedContext = contextMock.getContext();
			ServletContext servletContext = contextMock.getServletContext();
			doReturn(servletContext).when(contextMock).getServletContext();
			ServletContextHandler.ServletScopedContext servletScopedContext2 = contextMock.getContext();
			doReturn(servletScopedContext, servletScopedContext2).when(contextMock).getContext();
			//Act Statement(s)
			target.doStart();
			//Assert statement(s)
			assertAll("result", () -> {
