...
class ConfigDataResourceNotFoundExceptionSapientGeneratedTest {
	...
	//Sapient generated method id: ${8930b87e-cea2-3b95-93f3-2bdf661979ac}, hash: 52AE27FA4EC30E504A5A9C70EFBC539E
	@Test()
	void throwIfDoesNotExist1WhenLocationIsNullThrowsConfigDataResourceNotFoundException() {
		 ...
		 */
		//Arrange Statement(s)
		File fileToCheckMock = mock(File.class);
		doReturn(false).when(fileToCheckMock).exists();
		//Act Statement(s)
		final ConfigDataResourceNotFoundException result = assertThrows(ConfigDataResourceNotFoundException.class, () -> {
			ConfigDataResourceNotFoundException.throwIfDoesNotExist(configDataResourceMock, fileToCheckMock);
		});
		//Assert statement(s)
		assertAll("result", () -> {
			assertThat(result, is(notNullValue()));
			verify(fileToCheckMock).exists();
		});
	}
