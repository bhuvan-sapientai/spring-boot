...
class ConfigurationPropertyNameSapientGeneratedTest {
	...
	void subNameWhenOffsetNotGreaterThanGetNumberOfElements() {
		...
		Function functionMock = mock(Function.class);
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			_assert.when(() -> Assert.notNull("{value=test}", "Name must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			ConfigurationPropertyName target = ConfigurationPropertyName.adapt("{value=test}", '{', functionMock);
			//Act Statement(s)
			ConfigurationPropertyName result = target.subName(1);
			//Assert statement(s)
			assertAll("result", () -> {
				assertThat(result, is(notNullValue()));
				_assert.verify(() -> Assert.notNull("{value=test}", "Name must not be null"), atLeast(1));
			});
		}
	}
	...
	//Sapient generated method id: ${cd3ab7e9-39dc-3956-9ae9-68e1fee39e1f}, hash: 264FA218DEA3EDE3CC5A1D0B93559A5D
	@Test()
