...
class ProfilesSapientGeneratedTest {
	...
	//Sapient generated method id: ${cffbf970-a697-3296-9d13-518d7ba78eaa}
	@Disabled()
	@Test()
	...
	//Sapient generated method id: ${b99f08d2-fb50-3b52-b75b-ee013f79d6d0}
	@Disabled()
	@Test()
	...
	//Sapient generated method id: ${129ae9ad-fa43-3e97-9819-16403b42169e}
	@Disabled()
	@Test()
	...
	void toStringTest() {
		...
		try (MockedStatic<CollectionUtils> collectionUtils = mockStatic(CollectionUtils.class, CALLS_REAL_METHODS)) {
			...
			Profiles target = spy(new Profiles(environmentMock, binderMock, collection));
			List<String> stringList = new ArrayList<>();
			doReturn(stringList).when(target).getActive();
			List<String> stringList2 = new ArrayList<>();
			doReturn(stringList2).when(target).getDefault();
			List<String> stringList3 = new ArrayList<>();
			doReturn(stringList3).when(target).getAccepted();
			//Act Statement(s)
			...
			assertAll("result", () -> {
				...
				collectionUtils.verify(() -> CollectionUtils.isEmpty(anyCollection()), atLeast(1));
				collectionUtils.verify(() -> CollectionUtils.isEmpty(anyList()), atLeast(2));
				verify(target, atLeast(1)).getActive();
				verify(target, atLeast(1)).getDefault();
				verify(target, atLeast(1)).getAccepted();
			});
		}
	}
