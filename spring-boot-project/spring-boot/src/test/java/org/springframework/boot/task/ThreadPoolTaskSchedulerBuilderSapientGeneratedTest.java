...
class ThreadPoolTaskSchedulerBuilderSapientGeneratedTest {
	...
	//Sapient generated method id: ${4093ce62-8361-3b14-be69-19a1177dbf57}, hash: BB172379CDF938F14C67F97A596F2AEA
	@Test()
	void customizers1WhenSetIsNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			...
			Duration duration = Duration.ofDays(0L);
			Set<ThreadPoolTaskSchedulerCustomizer> threadPoolTaskSchedulerCustomizerSet = new HashSet<>();
			threadPoolTaskSchedulerCustomizerSet.add(threadPoolTaskSchedulerCustomizerMock);
			ThreadPoolTaskSchedulerBuilder target = new ThreadPoolTaskSchedulerBuilder(0, false, duration, "String", threadPoolTaskSchedulerCustomizerSet);
			//Act Statement(s)
			ThreadPoolTaskSchedulerBuilder result = target.customizers(additionsMock);
			//Assert statement(s)
			//TODO: Please implement equals method in ThreadPoolTaskSchedulerBuilder for verification of the entire object or you need to adjust respective assertion statements
			assertAll("result", () -> {
	...
	//Sapient generated method id: ${fbe004e5-01a4-3dff-881a-cf633dd17d5f}, hash: 5CB880B656DA97C4FEC7D55BD246EE8C
	@Test()
	void additionalCustomizers1WhenSetIsNotNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			...
			Set<ThreadPoolTaskSchedulerCustomizer> threadPoolTaskSchedulerCustomizerSet = new HashSet<>();
			threadPoolTaskSchedulerCustomizerSet.add(threadPoolTaskSchedulerCustomizerMock);
			ThreadPoolTaskSchedulerBuilder target = new ThreadPoolTaskSchedulerBuilder(0, false, duration, "<String value>", threadPoolTaskSchedulerCustomizerSet);
			//Act Statement(s)
			ThreadPoolTaskSchedulerBuilder result = target.additionalCustomizers(additionsMock);
			//Assert statement(s)
			//TODO: Please implement equals method in ThreadPoolTaskSchedulerBuilder for verification of the entire object or you need to adjust respective assertion statements
			assertAll("result", () -> {
				assertThat(result, is(notNullValue()));
	...
	//Sapient generated method id: ${ec4a4216-0b11-3482-8467-7bedcdd174bc}, hash: 003A77EAB76FD8A16EF1593783A511B1
	@Test()
	void additionalCustomizers1WhenSetIsNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			...
			_assert.when(() -> Assert.notNull(additionsMock, "Customizers must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			Duration duration = Duration.ofDays(0L);
			ThreadPoolTaskSchedulerBuilder target = new ThreadPoolTaskSchedulerBuilder(0, false, duration, "<String value>", (Set) null);
			//Act Statement(s)
			ThreadPoolTaskSchedulerBuilder result = target.additionalCustomizers(additionsMock);
			//Assert statement(s)
			//TODO: Please implement equals method in ThreadPoolTaskSchedulerBuilder for verification of the entire object or you need to adjust respective assertion statements
	...
	//Sapient generated method id: ${e9165558-da0a-36f1-a4c1-97dc35ed89ac}, hash: 886687C111A7549F5EAC80D90C54154D
	@Disabled()
	@Test()
