...
class ApplicationPidSapientGeneratedTest {
	...
	//Sapient generated method id: ${91cd37bb-3a3f-3516-9748-a67546b79a59}, hash: 6F7DB9EF99ED6721C1374C55350CDA8C
	@Disabled()
	@Test()
	void writeWhenCanWritePosixFileFile() throws Throwable {
		...
		File file = new File("pathname1");
		doReturn(file).when(fileMock).getParentFile();
		doReturn(true).when(fileMock).exists();
		doReturn(true).when(fileMock).canWrite();
		Path pathMock = mock(Path.class);
		doReturn(pathMock).when(fileMock).toPath();
		ApplicationPid target = new ApplicationPid("{value=1234}");
		//Act Statement(s)
		target.write(fileMock);
		//Assert statement(s)
		assertAll("result", () -> {
			verify(fileMock).getParentFile();
			verify(fileMock).exists();
			verify(fileMock).canWrite();
			verify(fileMock).toPath();
		});
	}
	...
	//Sapient generated method id: ${42b10ac1-f960-3d39-b8e5-9de93c41e390}, hash: 17EC29C32185EFCA1C56D2CD94861F7D
	@Disabled()
	@Test()
	void writeWhenCaughtUnsupportedOperationExceptionAndCanWritePosixFileFile() throws Throwable {
		...
		try (MockedStatic<Files> files = mockStatic(Files.class)) {
			...
			files.when(() -> Files.getPosixFilePermissions((Path) any(), eq(linkOptionArray))).thenReturn(posixFilePermissionSet);
			ApplicationPid target = new ApplicationPid("1234");
			File file = new File("pathname1");
			//Act Statement(s)
			target.write(file);
			//Assert statement(s)
			assertAll("result", () -> files.verify(() -> Files.getPosixFilePermissions((Path) any(), eq(linkOptionArray))));
		}
	}
}
