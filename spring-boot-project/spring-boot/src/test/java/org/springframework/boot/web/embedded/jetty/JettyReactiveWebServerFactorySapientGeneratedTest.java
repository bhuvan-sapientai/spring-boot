...
class JettyReactiveWebServerFactorySapientGeneratedTest {
	...
	void getWebServerWhenGetPortGreaterThanOrEqualsTo0() {
		...
		JettyWebServer jettyWebServer = new JettyWebServer(serverMock, false);
		//Assert statement(s)
		...
		assertAll("result", () -> {
			assertThat(result, equalTo(jettyWebServer));
	...
	//Sapient generated method id: ${0eb39326-5478-3097-ab16-d004cf1e1fa6}
	@Disabled()
	@Test()
	void createJettyServerWhenGetShutdownEqualsShutdownGRACEFUL() {
		...
		try (MockedStatic<WebServerSslBundle> webServerSslBundle = mockStatic(WebServerSslBundle.class);
			...
			//Assert statement(s)
			//TODO: Please implement equals method in Server for verification to succeed or you need to adjust respective assertion statements
			assertAll("result", () -> {
				assertThat(result, equalTo(server));
