...
class BinderSapientGeneratedTest {
	...
	void bind4WhenNotCreate() throws Exception {
		...
		try (MockedStatic<BindResult> bindResult = mockStatic(BindResult.class);
			 MockedStatic<Assert> _assert = mockStatic(Assert.class);
			 MockedStatic<BindConverter> bindConverter = mockStatic(BindConverter.class)) {
			doReturn(null).when(handlerMock).onStart(eq(configurationPropertyNameMock), eq(bindableMock), (Binder.Context) any());
			doNothing().when(handlerMock).onFinish(eq(configurationPropertyNameMock), eq(bindableMock), (Binder.Context) any(), eq((Object) null));
			_assert.when(() -> Assert.notNull((Iterable) null, "Sources must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			_assert.when(() -> Assert.notNull(configurationPropertySourceMock, "Sources must not contain null elements")).thenAnswer((Answer<Void>) invocation -> null);
			bindConverter.when(() -> BindConverter.get(anyList(), eq((Consumer) null))).thenReturn(bindConverterMock);
			...
			Binder target = new Binder((Iterable) null, placeholdersResolverMock, conversionServiceMock, (Consumer) null, bindHandlerMock, (BindConstructorProvider) null);
			//Act Statement(s)
			BindResult result = target.bind(configurationPropertyNameMock, bindableMock, handlerMock);
			//Assert statement(s)
			assertAll("result", () -> {
				assertThat(result, equalTo(bindResultMock));
				verify(handlerMock).onStart(eq(configurationPropertyNameMock), eq(bindableMock), (Binder.Context) any());
				verify(handlerMock).onFinish(eq(configurationPropertyNameMock), eq(bindableMock), (Binder.Context) any(), eq((Object) null));
				_assert.verify(() -> Assert.notNull((Iterable) null, "Sources must not be null"), atLeast(1));
				_assert.verify(() -> Assert.notNull(configurationPropertySourceMock, "Sources must not contain null elements"), atLeast(1));
				bindConverter.verify(() -> BindConverter.get(anyList(), eq((Consumer) null)));
	...
	void bind4WhenCaughtException() throws Exception {
		...
		try (MockedStatic<BindResult> bindResult = mockStatic(BindResult.class);
			...
			doReturn(object).when(handlerMock).onFailure(eq(configurationPropertyNameMock), eq(bindableMock), (Binder.Context) any(), eq(exception));
			_assert.when(() -> Assert.notNull((Iterable) null, "Sources must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			_assert.when(() -> Assert.notNull(configurationPropertySourceMock, "Sources must not contain null elements")).thenAnswer((Answer<Void>) invocation -> null);
			bindConverter.when(() -> BindConverter.get(anyList(), eq((Consumer) null))).thenReturn(bindConverterMock);
			_assert.when(() -> Assert.notNull(configurationPropertyNameMock, "Name must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			...
			assertAll("result", () -> {
				...
				verify(handlerMock).onStart(eq(configurationPropertyNameMock), eq(bindableMock), (Binder.Context) any());
				verify(handlerMock).onFinish(eq(configurationPropertyNameMock), eq(bindableMock), (Binder.Context) any(), eq((Object) null));
				verify(handlerMock).onFailure(eq(configurationPropertyNameMock), eq(bindableMock), (Binder.Context) any(), eq(exception));
				_assert.verify(() -> Assert.notNull((Iterable) null, "Sources must not be null"), atLeast(1));
				_assert.verify(() -> Assert.notNull(configurationPropertySourceMock, "Sources must not contain null elements"), atLeast(1));
				bindConverter.verify(() -> BindConverter.get(anyList(), eq((Consumer) null)));
				_assert.verify(() -> Assert.notNull(configurationPropertyNameMock, "Name must not be null"), atLeast(1));
	...
	void bind4WhenNotCreateAndCaughtException() throws Exception {
		...
		try (MockedStatic<BindResult> bindResult = mockStatic(BindResult.class);
			...
			Object object2 = new Object();
			bindResult.when(() -> BindResult.of(object2)).thenReturn(bindResultMock);
			Binder target = new Binder((Iterable) null, placeholdersResolverMock, conversionServiceMock, (Consumer) null, handlerMock, (BindConstructorProvider) null);
	...
	void bindOrCreate3WhenResultIsNull() throws Exception {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class);
			...
			Object object2 = new Object();
			Exception exception = new Exception();
			doReturn(object2).when(handlerMock).onFailure(eq(configurationPropertyNameMock), eq(targetMock), (Binder.Context) any(), eq(exception));
			_assert.when(() -> Assert.notNull((Iterable) null, "Sources must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			_assert.when(() -> Assert.notNull(configurationPropertySourceMock, "Sources must not contain null elements")).thenAnswer((Answer<Void>) invocation -> null);
			bindConverter.when(() -> BindConverter.get(anyList(), eq((Consumer) null))).thenReturn(bindConverterMock);
			_assert.when(() -> Assert.notNull(configurationPropertyNameMock, "Name must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			...
			assertAll("result", () -> {
				...
				verify(handlerMock).onFailure(eq(configurationPropertyNameMock), eq(targetMock), (Binder.Context) any(), eq(exception));
				_assert.verify(() -> Assert.notNull((Iterable) null, "Sources must not be null"), atLeast(1));
				_assert.verify(() -> Assert.notNull(configurationPropertySourceMock, "Sources must not contain null elements"), atLeast(1));
				bindConverter.verify(() -> BindConverter.get(anyList(), eq((Consumer) null)));
				_assert.verify(() -> Assert.notNull(configurationPropertyNameMock, "Name must not be null"), atLeast(1));
