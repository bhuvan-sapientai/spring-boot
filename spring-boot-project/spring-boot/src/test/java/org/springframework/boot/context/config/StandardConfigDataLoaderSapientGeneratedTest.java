...
class StandardConfigDataLoaderSapientGeneratedTest {
	...
	//Sapient generated method id: ${e33a41e9-2b70-3d61-8a58-9954526f5c6b}
	@Disabled()
	@Test()
	void loadWhenResourceGetProfileIsNotNull() throws IOException, ConfigDataNotFoundException {
		...
		try (MockedStatic<OriginTrackedResource> originTrackedResource = mockStatic(OriginTrackedResource.class);
			...
			//Act Statement(s)
			ConfigData result = target.load(configDataLoaderContextMock, resourceMock);
			ConfigData.Option[] optionArray = new ConfigData.Option[] { ConfigData.Option.PROFILE_SPECIFIC };
			ConfigData.PropertySourceOptions propertySourceOptions = ConfigData.PropertySourceOptions.always(optionArray);
			ConfigData configData = new ConfigData(anyList, propertySourceOptions);
			//Assert statement(s)
			//TODO: Please implement equals method in ConfigData for verification to succeed or you need to adjust respective assertion statements
			assertAll("result", () -> {
				assertThat(result, equalTo(configData));
	...
	//Sapient generated method id: ${e1a160d5-022e-389a-903a-30984214f4a1}
	@Disabled()
	@Test()
	void loadWhenResourceGetProfileIsNull() throws IOException, ConfigDataNotFoundException {
		...
		try (MockedStatic<OriginTrackedResource> originTrackedResource = mockStatic(OriginTrackedResource.class);
			...
			//Act Statement(s)
			ConfigData result = target.load(configDataLoaderContextMock, resourceMock);
			ConfigData.PropertySourceOptions propertySourceOptions = ConfigData.PropertySourceOptions.ALWAYS_NONE;
			ConfigData configData = new ConfigData(anyList, propertySourceOptions);
			//Assert statement(s)
			//TODO: Please implement equals method in ConfigData for verification to succeed or you need to adjust respective assertion statements
			assertAll("result", () -> {
