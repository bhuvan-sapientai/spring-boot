...
class BufferedStartupStepSapientGeneratedTest {
	...
	private final Consumer recorderMock = mock(Consumer.class, "null");
	...
	//Sapient generated method id: ${1492b9c2-b980-3a4a-9204-051aa921c1c9}, hash: 2C9A596561BFD20FCD93C480B8461115
	@Test()
	void tagTest() {
		//Arrange Statement(s)
		Supplier valueMock = mock(Supplier.class, "some_value");
		doReturn(null).when(valueMock).get();
		Instant instant = Instant.now();
		BufferedStartupStep target = spy(new BufferedStartupStep(parentMock, "step_name", 456L, instant, recorderMock));
		StartupStep startupStepMock = mock(StartupStep.class);
		doReturn(startupStepMock).when(target).tag("some_key", (String) null);

		//Act Statement(s)
		StartupStep result = target.tag("some_key", valueMock);

		//Assert statement(s)
		assertAll("result", () -> {
			assertThat(result, equalTo(startupStepMock));
			verify(valueMock).get();
			verify(target).tag("some_key", (String) null);
		});
	}
