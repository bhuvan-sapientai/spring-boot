...
class PemContentSapientGeneratedTest {
	...
	//Sapient generated method id: ${2b216bd1-213c-3ae7-8203-1ee6c92eb640}, hash: 369E5432783A5CD9B0D9B61CA4EF0AD9
	@Disabled()
	@Test()
	void loadWhenDefaultBranchThrowsThrowable() throws IOException, FileNotFoundException {
		...
		try (MockedStatic<PemContent> pemContent = mockStatic(PemContent.class, CALLS_REAL_METHODS);
			...
			pemContent.when(() -> PemContent.isPresentInText("example content")).thenReturn(false);
			resourceUtils.when(() -> ResourceUtils.getURL("example content")).thenReturn(uRLMock);
			...
			final Throwable result = assertThrows(Throwable.class, () -> {
				PemContent.load("example content");
			});
			//Assert statement(s)
			assertAll("result", () -> {
				assertThat(result, is(notNullValue()));
				pemContent.verify(() -> PemContent.isPresentInText("example content"), atLeast(1));
				resourceUtils.verify(() -> ResourceUtils.getURL("example content"), atLeast(1));
