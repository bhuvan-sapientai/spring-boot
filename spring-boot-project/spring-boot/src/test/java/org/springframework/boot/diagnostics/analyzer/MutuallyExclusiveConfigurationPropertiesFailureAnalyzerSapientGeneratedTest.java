...
class MutuallyExclusiveConfigurationPropertiesFailureAnalyzerSapientGeneratedTest {
	...
	//Sapient generated method id: ${3ae87aff-95b6-3319-9eb1-3e60f26478c5}
	@Test()
	...
	//Sapient generated method id: ${73e72d81-eef6-3c3f-9c7b-d18087f94179}
	@Disabled()
	@Test()
	void analyze2WhenInputIsNotEmpty() {
		...
		//Act Statement(s)
		FailureAnalysis result = target.analyze(throwable, causeMock);
		FailureAnalysis failureAnalysis = new FailureAnalysis("The following configuration properties are mutually exclusive:\n\n\tA\n\nHowever, more than one of those properties has been configured at the same time:\n\n", "Update your configuration so that only one of the mutually exclusive properties is configured.", causeMock);
		//Assert statement(s)
		...
		assertAll("result", () -> {
			...
			verify(causeMock).getConfiguredNames();
			verify(causeMock).getMutuallyExclusiveNames();
