...
class ThreadPoolTaskExecutorBuilderSapientGeneratedTest {
	...
	//Sapient generated method id: ${4093ce62-8361-3b14-be69-19a1177dbf57}, hash: EBCB74FC85B26ECA2E53DDB14BA6E199
	@Test()
	void customizers1WhenSetIsNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			...
			ThreadPoolTaskExecutorBuilder target = new ThreadPoolTaskExecutorBuilder();
			//Act Statement(s)
			ThreadPoolTaskExecutorBuilder result = target.customizers(additionsMock);
			//Assert statement(s)
			assertAll("result", () -> {
	...
	//Sapient generated method id: ${ec4a4216-0b11-3482-8467-7bedcdd174bc}, hash: 55C039CC4D92537902913DC3DB90E296
	@Test()
	void additionalCustomizers1WhenSetIsNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			...
			ThreadPoolTaskExecutorBuilder target = new ThreadPoolTaskExecutorBuilder();
			//Act Statement(s)
			ThreadPoolTaskExecutorBuilder result = target.additionalCustomizers(additionsMock);
			//Assert statement(s)
			assertAll("result", () -> {
				assertThat(result, is(notNullValue()));
				verify(additionsMock).forEach((Consumer) any());
