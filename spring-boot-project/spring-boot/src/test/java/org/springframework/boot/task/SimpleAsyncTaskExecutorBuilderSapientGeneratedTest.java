...
class SimpleAsyncTaskExecutorBuilderSapientGeneratedTest {
	...
	//Sapient generated method id: ${4093ce62-8361-3b14-be69-19a1177dbf57}, hash: 601C714FBB14A5DFA02EDA0B829F457F
	@Test()
	void customizers1WhenSetIsNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			...
			SimpleAsyncTaskExecutorBuilder target = new SimpleAsyncTaskExecutorBuilder();
			//Act Statement(s)
			SimpleAsyncTaskExecutorBuilder result = target.customizers(additionsMock);
			//Assert statement(s)
			assertAll("result", () -> {
	...
	//Sapient generated method id: ${ec4a4216-0b11-3482-8467-7bedcdd174bc}, hash: B741B5B5F05AB2390A9C64D03E2AF150
	@Test()
	void additionalCustomizers1WhenSetIsNull() {
		...
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class)) {
			...
			SimpleAsyncTaskExecutorBuilder target = new SimpleAsyncTaskExecutorBuilder();
			//Act Statement(s)
			SimpleAsyncTaskExecutorBuilder result = target.additionalCustomizers(additionsMock);
			//Assert statement(s)
			assertAll("result", () -> {
				assertThat(result, is(notNullValue()));
				verify(additionsMock).forEach((Consumer) any());
