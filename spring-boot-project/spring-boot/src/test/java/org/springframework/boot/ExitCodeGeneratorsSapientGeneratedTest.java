...
class ExitCodeGeneratorsSapientGeneratedTest {
	...
	//Sapient generated method id: ${3cebba44-d4fc-3122-b2c3-83b8bfdc1e68}, hash: C25FC46108DF9F7714FF0E0C7B7D8B27
	@Disabled()
	@Test()
	void addAll1WhenMappersIsNotEmpty() {
		 ...
		 */
		//Arrange Statement(s)
		Iterable mappersMock = mock(Iterable.class);
		try (MockedStatic<Assert> _assert = mockStatic(Assert.class, CALLS_REAL_METHODS)) {
			doReturn(null).when(mappersMock).iterator();
			_assert.when(() -> Assert.notNull(mappersMock, "Mappers must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			ExitCodeGenerators target = spy(new ExitCodeGenerators());
			Throwable throwable = new Throwable();
			doNothing().when(target).add(throwable, exitCodeExceptionMapperMock);
			//Act Statement(s)
			target.addAll(throwable, mappersMock);
			//Assert statement(s)
			assertAll("result", () -> {
				verify(mappersMock, atLeast(1)).iterator();
				_assert.verify(() -> Assert.notNull(mappersMock, "Mappers must not be null"), atLeast(1));
				verify(target, atLeast(1)).add(throwable, exitCodeExceptionMapperMock);
			});
		}
	}

	//Sapient generated method id: ${2f52d914-224e-3cf8-a2e5-c61089a0f6f4}, hash: 461992AECCC0D336F7FA81FE3267B7EE
	@Test()
	void addTest() {
		/*
		 * TODO: Help needed! Please adjust the input/test parameter values manually to satisfy the requirements of the given test scenario.
	...
	//Sapient generated method id: ${07efa7de-7358-38b4-9119-e0d9001c9353}, hash: 69428BA152F94A8EAA2078E16CA897D3
	@Disabled()
	@Test()
	void addAll3WhenGeneratorsIsNotEmpty() {
		...
		try (MockedStatic<AnnotationAwareOrderComparator> annotationAwareOrderComparator = mockStatic(AnnotationAwareOrderComparator.class);
			...
			_assert.when(() -> Assert.notNull(iterableMock, "Generators must not be null")).thenAnswer((Answer<Void>) invocation -> null);
			...
			ExitCodeGenerators target = new ExitCodeGenerators();
			//Act Statement(s)
			target.addAll(iterableMock);
			//Assert statement(s)
			assertAll("result", () -> {
				_assert.verify(() -> Assert.notNull(iterableMock, "Generators must not be null"), atLeast(1));
	...
	//Sapient generated method id: ${78c09fb6-edcf-3ce7-9932-6ef4ffc152b3}, hash: 00182DFE3D82885DD34C93248E7E5F7B
	@Test()
	void iteratorTest() {
		//Arrange Statement(s)
		ExitCodeGenerators target = new ExitCodeGenerators();
